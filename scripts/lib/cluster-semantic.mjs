...
import { isNearDuplicateTitle, longestCommonSubstring } from "./html.mjs";
...
/**
 * 摘要近重复判定(转载辅助证据,零词表):归一化后 LCS ≥ 20 字符 且 占较短摘要 ≥ 40%。
 * 同源转载的引擎摘要(正文首段)高度重叠;不同文章即使同主题摘要也不同。
 */
export function isNearDuplicateDesc(a, b) {
  let da = String(a || "").replace(/\s+/g, "");
  let db = String(b || "").replace(/\s+/g, "");
  if (da.length < 20 || db.length < 20) return false;
  // 防御上限:滚动数组已内存安全(O(m)),但 4k+ 字符的 LCS 时间开销陡增
  // (实测 64k 字符对 ≈ 40 亿次循环,几十秒);转载判定特征在前部,超长截断 4096 足够
  if (da.length > 4096) da = da.slice(0, 4096);
  if (db.length > 4096) db = db.slice(0, 4096);
  const lcs = longestCommonSubstring(da, db);
  return lcs >= 20 && lcs / Math.min(da.length, db.length) >= 0.4;
}

/** 转载文本证据:标题或摘要任一近重复(与向量余弦组合使用,见 cluster.mjs 预处理) */
export function hasReprintTextEvidence(a, b) {
  return isNearDuplicateTitle(a.title, b.title) || isNearDuplicateDesc(a.desc, b.desc);
}
...
function greedyClusterByCentroid(docs, opts) {
  ...
  for (const d of docs) {
    ...
    if (best && bestSim >= opts.simThreshold) {
      ...
      clusters.push({ members: [d], centroid: [...d.vec], dups: 0, tokens: [...d.tokens] });
    }
  }
  return clusters;
}
...
function makeClusters(groups) {
  return groups.map((g) => {
    const cl = { members: g, centroid: null, dups: 0, tokens: g.flatMap((d) => [...d.tokens]) };
 ...
 *
 * 截止(零固定阈值):
 *   - 绝对下限:合并高度 < 单例集合 pairwise 的 Q3(75 分位)不合并 —— 低于
 *     Q3 的相似度在单例集合里"太普通"不构成桶;单例互不相关时 Q3 低,自然零合并
 ...
 * 桶名走 readableClusterLabel(桶内共享短语/回退)。零固定阈值(见 upgmaBuckets)。
 */
function bucketSingletons(clusters, maxBucketSize) {
  ...
  const buckets = upgmaBuckets(items.length, sim, maxBucketSize);
  ...
  for (const b of buckets) {
    ...
    const cl = { members, centroid: null, dups: 0, tokens: members.flatMap((d) => [...d.tokens]) };
    recomputeCentroid(cl);
    out.push(cl);
  }
  return out;
...
export function buildSemantic(docs, opts) {
  ...
  if (opts.bucketSingletons) clusters = bucketSingletons(clusters, opts.maxBucketSize);
  return clusters;
}
