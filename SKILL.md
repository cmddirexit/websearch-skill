...
- 每次搜索自动学习域名自身信誉;跨域 token 模式只从正文结构或显式 LLM 判断等独立内容证据学习,持久化在
  `~/.cache/websearch-domain-rep.json`(跨进程积累,无需清理)
- 学习信号:默认本地 quality/低质标记只更新当前域名,不反训跨域 token(避免目标泄漏);
  404/空壳只更新抓取可用性,也不训练内容模型;完整正文可由本地结构证据生成保守标签;
  主动 `fetch` 只更新独立使用价值分(同 URL 每日去重,三次后最多微升约 3%),不代表内容可信,
  不训练跨域模型,未被 `fetch` 也不是负反馈;
  LLM 内容可信度判断是显式选择的可选增强功能
  (`WEBSEARCH_LLM_ENABLED=1` + `WEBSEARCH_LLM_PROVIDER=...`),未启用时不会外发搜索摘要或正文片段;
  不依赖 LLM 也能积累跨域证据
- 跨域模型需同时积累足够正负样本才启用;标题/URL 特征按通道归一化,域名身份和规则 flag
  不进入跨域模型;域名自身证据增加后先验平滑淡出
...
- **`[meta:x.xx]` 冷启动预测**:新域名,无自身样本,模式预测分 —— 仅供参考,
  不可靠时以 fetch 实测为准
- **`[blend:x.xx]` 融合预测**:已有少量自身样本,仍与跨域先验渐进融合
...
fetch 回写内容证据时按`结构负证据 → 语义反证 → 结构正证据 → 贝叶斯`裁决;
结构正证据不能绕过标题-正文语义检查。语义嵌入严格服从 `off|api|local|wasm|auto`
后端配置，`off` 不外发正文，API 使用短超时。全模棱两可则中性不训练。
